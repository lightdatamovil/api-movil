...
export async function crearLog(empresa, usuario, perfil, body, tiempo, resultado, endpoint, exito) {
    try {
        ...
        logGreen(`Log creado${endpoint.contains("/shipment-list") || endpoint.contains("/driver-list") || endpoint.contains("/company-identification") ? `: ${JSON.stringify(values)}` : " con éxito"}`);
    } catch (error) {
