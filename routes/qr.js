...
import { getShipmentIdFromQrLocal, crossDocking, driverList, getSkuAndNumberOfItems, getProductsFromShipment, enterFlex } from "../controller/qrController.js";
...
qr.post('/sku', verifyToken, async (req, res) => {
    ...
    try {
        ...
        logRed(`Error en la ruta /sku: ${error.stack}`);
        res.status(500).json({ message: error.stack });
    } finally {
        const endTime = performance.now();
        logPurple(`Tiempo de ejecución: ${endTime - startTime} ms`);
