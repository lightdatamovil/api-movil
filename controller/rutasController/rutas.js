    ...
    try {
        ...
        if (rutaResult.length > 0) {
            ...
            for (let row of getRouteShipmentsQueryResult) {
                let latitud = row.destination_latitude ? parseFloat(row.destination_latitude) : null;
                let longitud = row.destination_longitude ? parseFloat(row.destination_longitude) : null;
                ...
                if (Adata.dataRuta.length === 0 && row.dataR) {
