...
export async function uploadImage(company, shipmentId, userId, shipmentState, image, lineId) {
    ...
    try {
        ...
        await executeQuery(dbConnection, insertQuery, [shipmentId, response.data, server, userId, lineId, shipmentState], true);
        ...
        if (companyId == 334 && trueSiNadie) {
            ...
            const updateQuery = "UPDATE envios_historial SET conFoto = 1 WHERE didEnvio = ? AND estado = ? and elim = 0 and superado = 0 ORDER BY fecha DESC LIMIT 1";
            await executeQuery(dbConnection, updateQuery, [shipmentId, shipmentState]);
        }
