...
export async function armado(company, userId, dataEnvios, didCliente, fecha) {
    ...
    try {
        const [row1] = await executeQuery(dbConnection, 'SELECT MAX(did) AS maxDid FROM fulfillment_movimientos_stock', [], true);
        let maxDid = (row1.maxDid || 0) + 1;

        const [row2] = await executeQuery(dbConnection, 'SELECT MAX(did) AS maxDidLinea FROM fulfillment_movimientos_stock_lineas', [], true);
        ...
        for (const item of dataEnvios) {
         ...
         VALUES (?, ?, ?, 0, ?, ?)`,
                [maxDidLinea, maxDid, item.didProducto, descuentoStr, userId], true
            );
            maxDidLinea++;
        ...
        await executeQuery(
         ...
         VALUES (?, ?, ?, -1, ?, '', ?, ?, ?)`,
            [maxDid, didCliente, fecha, dataEnvios[dataEnvios.length - 1]?.did, lineas, cantidad, userId], true
        );
